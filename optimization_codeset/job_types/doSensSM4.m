...
tp{1} = 'giv'; %generator interventions
ta(1,:) = linspace(0,1.8,n);
tp{2} = 'fco'; %fuel cost
ta(2,:) = linspace(0.6,2.4,n);
tp{3} = 'fca'; %fuel capacity
ta(3,:) = linspace(400,2200,n);
tp{4} = 'fsl'; %fuel shelf life
ta(4,:) = linspace(6,60,n);
...
giv = doSens(ta(1,:),tp{1},batchtype,batchpm,batchscen,batchloc,batchc);
fco = doSens(ta(2,:),tp{2},batchtype,batchpm,batchscen,batchloc,batchc);
fca = doSens(ta(3,:),tp{3},batchtype,batchpm,batchscen,batchloc,batchc);
