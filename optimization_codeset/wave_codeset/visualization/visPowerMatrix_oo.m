function [] = visPowerMatrix_oo(Hsm,Tpm,rated,wave)
...
c0 = [0.5 60];
fun = @(c)findSkewedSS_oo(linspace(x(1),x(end),N),c,wave,Tpm);
options = optimset('MaxFunEvals',10000,'MaxIter',10000, ...
    'TolFun',.0001,'TolX',.0001);
tic
c = fminsearch(fun,c0,options);
[~,y_,Y,prob_max] = findSkewedSS_oo(linspace(x(1),x(end),N),c,wave,Tpm);
%figure, plot(y_,Y)
toc
...
tp_eff_r = ...
    skewedGaussian_oo(wave.tp_res*Tpm,c(1),c(2),prob_max); %Tp eff (resonance)
