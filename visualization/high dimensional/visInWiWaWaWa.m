function [] = visInWiWaWaWa(pm1,pm2,pm3,pm4,pm5)

allStruct = mergeInWiWaWaWa(pm1,pm2,pm3,pm4,pm5);

np = 5; %number of power modules
...
for loc = 1:nl
    for pm = 1:np
        for c = 1:nu
            ...
            if pm == 3 || pm == 4 || pm == 5 %wave-specific 
                costdata(loc,pm,2,c) = ... %gen capex
                    allStruct(loc,pm,c).output.min.kWcost/1000 + ...
                    allStruct(loc,pm,c).output.min.Icost/1000 + ...
                    allStruct(loc,pm,c).output.min.Wecmoor/1000; 
                costdata(loc,pm,4,c) = ... %gen opex
...
leg = {'Platform','Generation CapEx','Storage CapEx','Generation OpEx', ...
    'Storage OpEx','Vessel'};

%colors
cols = 6;
col(1,:) = [0,0,51]/256; %platform cost
col([2 4],:) = flipud(colormap(brewermap(2,'purples'))); %generation cost
col([3 5],:) = flipud(colormap(brewermap(2,'blues'))); %storage cost
