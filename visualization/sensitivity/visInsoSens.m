...
Strcost = zeros(1,length(multStruct));
Pmtrl = zeros(1,length(multStruct));
Pinst = zeros(1,length(multStruct));
Pmoor = zeros(1,length(multStruct));
...
for i = 1:length(multStruct)
    ...
    Strcost(i) = multStruct(i).output.min.Strcost;
    Pmtrl(i) = multStruct(i).output.min.Pmtrl;
    Pinst(i) = multStruct(i).output.min.Pinst;
    Pmoor(i) = multStruct(i).output.min.Pmoor;
    ...
    dp(i) = multStruct(i).output.min.dp;
end
...
if exist('yscale','var')
    yscale = (1/1000)*yscale;
else
    yscale = 1/1000; 
end
if exist('xscale','var'), xscale = xscale; else, xscale = 1; end
a = area(multStruct(1).opt.tuning_array.*xscale, ... 
    [Mcost;Scost;Ecost;Icost;Strcost;Pmtrl;Pinst;Pmoor;battencl; ... 
    battreplace;vesselcost]'.*yscale);
...
xticks(xt)
legend('CapEx: Module','CapEx: Storage','CapEx: Electrical', ... 
    'CapEx: Installation','CapEx: Structural', ...
    'CapEx: Platform Material','CapEx: Platform Installation', ...
    'CapEx: Platform Mooring','CapEx: Battery Enclosure', ... 
    'OpEx: Battery Replacements', ... 
...
%storage
ax(3) = subplot(6,1,5);
plot(multStruct(1).opt.tuning_array,Smax, ... 
    'k','LineWidth',1.6,'DisplayName', ...
    'Storage Capacity')
ylabel('[kWh]')
...
%diameter
ax(4) = subplot(6,1,6);
plot(multStruct(1).opt.tuning_array,dp, ... 
    'r','LineWidth',1.6,'DisplayName', ...
    'Platform Diameter')
ylabel('[m]')
ylim([0 1.25*max(dp)])
xticks(xt)
