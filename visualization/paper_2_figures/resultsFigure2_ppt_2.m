        ...
        for c = 1:nu
            ...
            if pm == 3 %wind-specific
                costdata(loc,pm,2,c) = costdata(loc,pm,2,c) + ... 
                    ...
                    allStruct(loc,pm,c).output.min.Icost/1000;
                costdata(loc,pm,3,c) = costdata(loc,pm,3,c) + ... 
...
%colors
% cols = 6;
mcol = [256,256,256]/256; %platform cost
% col([2 4],:) = flipud(brewermap(2,'purples')); %generation cost
% col([3 5],:) = flipud(brewermap(2,'blues')); %storage cost
vcol = [238,232,170]/256; %vessel cost
cols = 4;
col(1,:,1) = [25,25,150]/256; %wave mooring
col(1,:,1) = mcol; %wave mooring
col(2,:,1) = [125,125,255]/256; %wave capex
col(3,:,1) = [200 200 255]/256; %wave opex
col(4,:,1) = [240 240 255]/256; %wave vessel
col(4,:,1) = vcol; %wave vessel
col(1,:,2) = [150 25 25]/256; %inso mooring
col(1,:,2) = mcol; %inso mooring
col(2,:,2) = [255 100 100]/256; %inso capex
col(3,:,2) = [255 170 170]/256; %inso opex
col(4,:,2) = [255 235 235]/256; %inso vessel
col(4,:,2) = vcol; %inso vessel
col(1,:,3) = [10 150 40]/256; %wind mooring
col(1,:,3) = mcol; %wind mooring
col(2,:,3) = [50 200 80]/256; %wind capex
col(3,:,3) = [200 255 200]/256; %wind opex
col(4,:,3) = [240 255 240]/256; %wind vessel
col(4,:,3) = vcol; %wind vessel
col(1,:,4) = [30 30 30]/256; %dies mooring
col(1,:,4) = mcol; %dies mooring
col(2,:,4) = [100 100 100]/256; %dies capex
col(3,:,4) = [175 175 175]/256; %dies opex
col(4,:,4) = [230 230 230]/256; %dies vessel
col(4,:,4) = vcol; %dies vessel
...
%         set(h(i,:,c),'XData',xdatpos);
        ...
        %set legend
        if c == 1 && i == np
            leg = legend(h(:,2,c),pms,'Location','northeast');
            leg.FontSize = fs4;
            leg.Position(1) = .35;
            leg.Position(2) = .85;
...
print(results,['~/Dropbox (MREL)/Research/Defense/' ...
    'presentation_figures/results_0'],'-dpng','-r600')
