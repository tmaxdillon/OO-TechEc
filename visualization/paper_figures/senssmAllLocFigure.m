clearvars -except array x0 t0 cost
...
%     tp{a} = array(a,1,4).opt.tuned_parameter; %tuned parameter
%     ta(a,:) = array(a,1,4).opt.tuning_array; %tuned array
% end
...
for f = 1:2
    ...
    for a = 1:size(array,1)
        ...
        for i = 1:5
            ...
            xl(i) = xline(x0(a,l(i)),'Color',glc,'Alpha',gla, ...
                'LineStyle',gls);
            blt = scatter(x0(a,l(i)),t0(l(i)),ms, ...
        ...
        ylim([0 max(cost(:))])
        yticks([0 500000])
        %yticklabels({'$0k',['$' num2str(round(t0(l(i))/1000),3) 'k']})
