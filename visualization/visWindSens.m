function [] = visWindSens(multStruct,xlab,xt)
...
CapCol = colormap(brewermap(4,'reds'));
...
a(5).FaceColor = OpCol(1,:);
a(6).FaceColor = OpCol(2,:);
a(7).FaceColor = OpCol(3,:);
a(8).FaceColor = OpCol(4,:);
ylabel('cost in thousands')
ylim([0 1.25*max(cost)/1000])
xticks(xt)
legend('CapEx: Storage','CapEx: Turbine','CapEx: Platform', ... 
    'CapEx: Installation','OpEx: Maintenance','OpEx: Fuel', ... 
